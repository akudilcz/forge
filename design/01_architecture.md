...
**Chunked judging for the combined quality check.** One call judging *every* candidate node cannot survive large phases: the judge's verdict block scales with node count, and past the provider's output-token limit the response truncates mid-list. Live evidence (priority_queue wave-2, phase 3): 81 HLRs in one call → ~19 verdicts returned, 62 nodes unjudged; the retry re-sent all 62 in one call, truncated identically, and the phase halted with `UnjudgedQualityError`. The loudness was correct — the economics were not: a phase above the truncation threshold could *never* pass. `run_combined_quality_check` therefore splits the post-cache candidate list into chunks of `LLMConfig.quality_judge_batch_size` (default 25) and judges each chunk independently through the existing checker (one call plus at most one partial-rejudge retry per chunk). Verdicts and gaps accumulate across chunks; `UnjudgedQualityError` is raised only when a *chunk* still has unjudged nodes after its own retry — and a retry now re-asks at most one chunk's worth of nodes, which fits the output budget. Sticky PASS verdicts are stamped per successfully judged chunk, so a failure in a later chunk never discards evidence already paid for in earlier chunks. Sets of at most `quality_judge_batch_size` nodes behave exactly as before: a single batched call.

**Hallucinated node ids in judge verdicts.** Unjudged accounting is computed strictly as candidate-set minus judged: verdict lines naming an id outside the candidate set are ignored (never a gap, never "missing"), the retry re-sends only real candidates, and `UnjudgedQualityError` can therefore only ever name real candidate ids. Dropped unknown-id lines are not silent: each parse emits one WARN naming the unknown ids, so a judge that hallucinates ids (e.g. `HLR-9999`) leaves forensic evidence in the logs instead of vanishing without trace.
...
**Bounded resilience for the dedup judge call.** A live build (online_statistics r4) halted at phase 8 when a single malformed provider response — a body the HTTP client could not parse as JSON (`JSONDecodeError: Expecting value: line 157 column 1 (char 858)`) — propagated raw out of `llm.ainvoke` in the dedup judge and crashed `run_semantic_check`, halting the whole build. Loud is correct; brittle is not. `_judge_once` therefore retries the LLM call **exactly once** on `json.JSONDecodeError`; if the fresh call also fails to parse, it raises `UnjudgedDedupError` (carrying the node id and a raw-body snippet). `run_semantic_check` catches that error **per candidate**: the node's verdict is UNJUDGED — the node is **kept** (never deleted on unparseable evidence, consistent with the double-confirmation safety), the failure is logged at ERROR with the raw snippet, and the sweep continues to the next candidate. At sweep end any unjudged candidates are reported loudly (count + node ids) in the completion log — but the phase is **not** halted: an unjudged dedup verdict means "no deletion", which is the safe outcome, unlike quality verdicts which gate completion (`UnjudgedQualityError` in `run_combined_quality_check` still propagates after its single retry).
...
| `combined_quality` | Batched LLM calls judge every authored node on four axes (ATOMIC, EARS, title↔content match, title specificity) and emits the relevant gap types. Candidates are judged in chunks of `LLMConfig.quality_judge_batch_size` (default 25) so a large phase never exceeds the judge's output-token budget (§7.4 "Chunked judging"). A missing verdict is never a pass: nodes/axes the model failed to judge are re-asked in exactly one follow-up call per chunk, and anything still unjudged raises `UnjudgedQualityError` — the step fails loudly rather than scoring silence as clean. PASS verdicts are sticky per `(node_id, content-hash)` on the flow, so unchanged nodes are not re-judged in later cycles; FAIL verdicts are never cached (see §7.4). Before dispatching fixes, batchable title/wording gaps go through the batched micro-repair pre-pass (§7.4); only gaps it could not certify-resolve are dispatched per-gap |
...
| `semantic` | Detect and remove semantic duplicate nodes (sibling-scoped; skips containers and sole-coverage children). Deletion requires the same DUPLICATE verdict from **two independent LLM calls** — a single nondeterministic verdict must not destroy requirement text. A UNIQUE verdict (including a UNIQUE on the confirmation call) is sticky: it is cached per `(node_id, content-hash)` on the flow, so unchanged nodes are never re-litigated by later pipeline cycles. A deterministic lexical prescreen (token-set overlap, stdlib only) skips clearly-dissimilar candidates before the LLM judge; it only reduces candidate pairs and never authorises deletion (see §7.4). A judge call whose provider response cannot be parsed (raw `JSONDecodeError` from the client) is retried once; if the retry also fails the candidate is UNJUDGED — kept, logged at ERROR, and the sweep continues; unjudged candidates are reported loudly (count + ids) at sweep end without halting the phase |
